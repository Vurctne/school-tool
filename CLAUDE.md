...
- **Cowork sandbox cross-filesystem divergence.** The Windows-side file tools (Read/Edit/Write) and the Linux bash mount can disagree about the contents of a recently-edited file — bash may see a truncated version (often with trailing null-byte padding to the original file size) while Read shows the full one. Symptoms: AST-parsing tests fail with `SyntaxError` at line numbers the Read tool says don't exist; bash `wc -l` returns a smaller line count than Read shows; `python3 -c "data = pathlib.Path(p).read_bytes(); print(data.count(b'\x00'))"` returns non-zero. Workarounds: (a) if bash sees the right content, `pathlib.Path(p).write_text(p.read_text())` from bash; (b) if bash is corrupt and Windows is correct, write a `/tmp` file with the full content from Read view + `cp /tmp/file path/to/file` to overwrite the bash mount. Then `touch <file>` to bump mtime — Python's import machinery uses mtime to decide whether to re-parse the source, and a stale mtime can cause already-imported `@dataclass` definitions to keep their old field set even after the source changes. Encountered repeatedly during Phase 3 dispatches (rounds 8a–8c, 2026-04-26).
...
- **Cloudflare Workers caps PBKDF2 iterations at 100,000.** Web Crypto's `subtle.deriveBits` rejects iteration counts above 100k with `NotSupportedError: Pbkdf2 failed: iteration counts above 100000 are not supported`. `backend/src/lib/argon2.ts` sits at the cap (was 310k pre-2026-04-26 — that value caused every `/v1/auth/register` call to 500). Encountered Round 7 (2026-04-26).
- **hash-wasm argon2id can fail silently in Workers production builds.** The WASM compile path appears restricted in some compatibility regimes — `argon2id()` throws and the catch falls through to PBKDF2. `argon2.ts` now logs `console.warn({event: "argon2.hash.fallback_pbkdf2", error: …})` so the error surfaces via `wrangler tail` instead of silently degrading. To get argon2 actually working: swap to `@noble/hashes/argon2` (pure JS, ~3-5× slower per hash, but no WASM), OR vendor the .wasm file and import it statically (`import wasm from './argon2.wasm'`). Tracked as a follow-up; the PBKDF2-100k path is fine for the pilot.
...
- **PowerShell 5.x `Out-File -Encoding utf8` writes UTF-8 WITH BOM.** When a commit message file is created via `'@...'@ | Out-File -Encoding utf8` and then fed to `git commit -F`, the BOM bytes (`EF BB BF`) become the first 3 bytes of the commit message subject. `git log` then renders the subject as `锘禤hase 3 ...` instead of `Phase 3 ...` (cosmetic only — body is unaffected, push/pull/web display all work). Workaround: use `[System.IO.File]::WriteAllText($path, $content, (New-Object System.Text.UTF8Encoding($false)))` instead of `Out-File -Encoding utf8`. PowerShell 6+ has `-Encoding utf8NoBOM` natively. Encountered 2026-04-26 on Phase 2 + Phase 3 commits — affected SHAs `834ea30` and `cd2fba7`; left as-is (cosmetic only) but future commits should use the BOM-free helper.
